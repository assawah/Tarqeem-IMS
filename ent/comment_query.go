...
func (cq *CommentQuery) QueryIssue() *IssueQuery {
	query := (&IssueClient{config: cq.config}).Query()
...
func (cq *CommentQuery) WithIssue(opts ...func(*IssueQuery)) *CommentQuery {
	query := (&IssueClient{config: cq.config}).Query()
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withIssue; query != nil {
		if err := cq.loadIssue(ctx, query, nodes,
			func(n *Comment) { n.Edges.Issue = []*Issue{} },
			func(n *Comment, e *Issue) { n.Edges.Issue = append(n.Edges.Issue, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadIssue(ctx context.Context, query *IssueQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Issue)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(comment.IssueTable)
		s.Join(joinT).On(s.C(issue.FieldID), joinT.C(comment.IssuePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(comment.IssuePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(comment.IssuePrimaryKey[1]))

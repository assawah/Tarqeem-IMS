...
func (iq *IssueQuery) QueryComments() *CommentQuery {
...
func (iq *IssueQuery) QueryFiles() *FileQuery {
...
func (iq *IssueQuery) WithComments(opts ...func(*CommentQuery)) *IssueQuery {
	query := (&CommentClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *IssueQuery) WithFiles(opts ...func(*FileQuery)) *IssueQuery {
	query := (&FileClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *IssueQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Issue, error) {
	...
	if query := iq.withFiles; query != nil {
		if err := iq.loadFiles(ctx, query, nodes,
			func(n *Issue) { n.Edges.Files = []*File{} },
			func(n *Issue, e *File) { n.Edges.Files = append(n.Edges.Files, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *IssueQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Issue, init func(*Issue), assign func(*Issue, *Comment)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(issue.CommentsTable)
		s.Join(joinT).On(s.C(comment.FieldID), joinT.C(issue.CommentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(issue.CommentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(issue.CommentsPrimaryKey[0]))
...
func (iq *IssueQuery) loadFiles(ctx context.Context, query *FileQuery, nodes []*Issue, init func(*Issue), assign func(*Issue, *File)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Issue)
	nids := make(map[int]map[*Issue]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(issue.FilesTable)
		s.Join(joinT).On(s.C(file.FieldID), joinT.C(issue.FilesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(issue.FilesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(issue.FilesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
